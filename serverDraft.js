...
const logTokenUsage = async (endpoint, usage, model) => {
  ...
  usageData.averageTokensPerRequest = Math.round(usageData.totalTokensUsed / usageData.requestsCount)
...
app.post('/api/grade-questions', async (req, res) => {
  try {
    ...
    if (response.data.usage) {
        logTokenUsage('/api/grade-questions', response.data.usage, 'gpt-4o-mini')
      }
    }
